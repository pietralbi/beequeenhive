...
local function CalcDodgeMult(self)
    ...
    local target = self.inst.components.combat.target
    if target then
        if self.inst:IsNear(target, TUNING.BEEQUEEN_ATTACK_RANGE + target:GetPhysicsRadius(0)) then
            if found then
                return 0.5
            end
            found = true
...
local function ShouldDodge(self)
    ...
    for i = 10, 20, 5 do
        ...
        local dtheta = PI * .25
        for attempt = 1, 8 do --position, start_angle, radius, attempts, check_los, ignore_walls
			local offset = FindWalkableOffset(pos, theta, r, 1, true, false)
            if offset ~= nil then
                ...
                if distsq(homepos.x, homepos.z, x, z) < maxrangesq then
                    ...
                    if player.entity:IsVisible() then
