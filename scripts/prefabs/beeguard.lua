...
local function RetargetFn(inst)
    ...
    local distsq = player and inst:GetDistanceSqToInst(player)
...
local function TryToFindQueen(inst)
    local queen = GetPlayer()
    if queen then
    ...
    if queen ~= data.commander then
        ...
        if totalbeeguards > 0 then
            ...
            for i, v in ipairs(allbeeguards) do
                local angle = 2*PI * (i - math.random()) / totalbeeguards
